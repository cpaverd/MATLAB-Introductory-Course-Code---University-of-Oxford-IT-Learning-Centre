...
%Note, you need the image processing toolbox to run this code.
%Error 1: The if statement that checks that there are circles found needed to have == instead
%of =
%Error 2: The title of the figure that displays the number of cells needed a num2str()
%to convert the number of cells into a variable of the 'character' class
%Error 3: The last for loop needed to go from 1 instead of 0. Matlab matrices always start at
%position 1, so if you say myMatrix(0) it will give you an out-of-bounds error message 
